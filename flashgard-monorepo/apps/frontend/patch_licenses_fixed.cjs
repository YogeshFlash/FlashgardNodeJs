...
// 2. We need to replace EVERYTHING from `const LicensesPage = () => {` down to `const groupedCredits = ... }, {});`
const startIdx = code.indexOf('const LicensesPage = () => {');
const endStr = '  const groupedCredits = cutCredits.reduce((acc: any, item) => {\n    if (!acc[item.batchId]) acc[item.batchId] = { info: item.batch, items: [] };\n    acc[item.batchId].items.push(item);\n    return acc;\n  }, {});';
const endIdx = code.indexOf(endStr);

if (startIdx === -1 || endIdx === -1) {
  console.error("COULD NOT FIND START OR END", {start: startIdx !== -1, end: endIdx !== -1});
  process.exit(1);
}
...
code = code.substring(0, startIdx) + newStateAndFetch + code.substring(endIdx + endStr.length);
...
code = code.replace(/filteredLicenses/g, 'orgLicenses');
