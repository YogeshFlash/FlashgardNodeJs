...
const Sidebar = ({ mobile = false, onClose }: { mobile?: boolean; onClose?: () => void }) => {
  const { user } = useAuth();
  return (
    <aside className={`${mobile ? 'w-full' : 'w-64 fixed left-0 top-0 h-screen'} bg-slate-900 text-slate-300 flex flex-col z-20`}>
      <div className="h-16 flex items-center justify-between px-6 border-b border-slate-800 bg-slate-950">
        <div className="flex items-center gap-3">
          <ShieldCheck className="w-6 h-6 text-blue-500" />
          <span className="text-lg font-bold text-white tracking-wide">Flashgard</span>
        </div>
        {mobile && (
      ...
      <nav className="flex-1 py-6 px-4 space-y-1 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            ...
            onClick={onClose}
            className={({ isActive }) =>
              `flex items-center px-3 py-2.5 rounded-lg transition-all font-medium text-sm gap-3
              ${isActive
      ...
      <div className="p-4 border-t border-slate-800">
        {user?.isSuperAdmin ? (
          ...
          <div className="mb-3 px-3 py-1.5 bg-blue-500/10 border border-blue-500/20 rounded-lg">
            <p className="text-blue-400 text-xs font-semibold">🏛️ {user.organization.name}</p>
          </div>
        )}
...
const Topbar = ({ onMenuClick }: { onMenuClick: () => void }) => {
  ...
  return (
    <header className="h-16 bg-white border-b border-slate-200 fixed top-0 right-0 left-64 z-10 flex items-center justify-between px-6 max-[768px]:left-0">
          ...
          >
            ...
            <div className="text-left hidden sm:block">
              <p className="text-sm font-medium text-slate-700 leading-tight">
                {user?.isSuperAdmin ? 'Super Admin' : user?.organization?.name || 'Admin'}
              </p>
...
const DashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="md:pl-64 pt-16 min-h-screen overflow-y-auto">
