...
for (let i = 0; i < lines.length; i++) {
    ...
    // In update2.js, licensesFile originalname is:
    // `licensesFile ? licensesFile.originalname : '',`
    // `licenseDealersFile ? licenseDealersFile.originalname : ''`
    if (lines[i].includes('licensesFile ? licensesFile.originalname : \'\'')) {
        lines[i] = lines[i].replace('licensesFile ? licensesFile.originalname : \'\'', 'licensesFile && !Array.isArray(licensesFile) ? (licensesFile as Express.Multer.File).originalname : \'\'');
    }
    if (lines[i].includes('licenseDealersFile ? licenseDealersFile.originalname : \'\'')) {
        lines[i] = lines[i].replace('licenseDealersFile ? licenseDealersFile.originalname : \'\'', 'licenseDealersFile && !Array.isArray(licenseDealersFile) ? (licenseDealersFile as Express.Multer.File).originalname : \'\'');
    }
